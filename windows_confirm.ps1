...
if ($server_exe -and (Test-Path $gguf)) {
  ...
  if ($ready) {
    ...
    $gpu = $log | Select-String -Pattern 'offloaded .* layers to GPU|CUDA0|ggml_cuda_init|using device CUDA' | Select-Object -First 4
    if ($gpu) { $gpu | ForEach-Object { Info $_.Line }; Ok 'CUDA GPU backend is ACTIVE (real GPU offload)' }
    else { Warn 'no CUDA device lines in the log - running on CPU (no GPU on this host, or backend did not load)'; ($log | Select-String -Pattern 'buffer size|backend|CPU' | Select-Object -First 3) | ForEach-Object { Info $_.Line } }
  } else {
